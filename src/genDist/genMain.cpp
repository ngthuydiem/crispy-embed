...
int main(int argc, char* argv[])
{
	...
	READ *readArray;	
	string readFileName, inFileName, pairFileName, outFileName;
	...
	inFileName.append(".pair");
	outFileName = readFileName;
	outFileName.append(".dist");
	...
	*/
	...
	if (useGPU) {
		...
		computeGenDist_CUDA(inPairFile, outFileName, readArray, numReads, maxLen, threshold, band);	
	}
	else {
		...
		if (band > 1)
			computeGenDist_CPU_band(inPairFile, outFileName, readArray, numReads, maxLen, threshold, band, numThreads);	
		else
			computeGenDist_CPU_full(inPairFile, outFileName, readArray, numReads, maxLen, threshold, numThreads);	
	}
