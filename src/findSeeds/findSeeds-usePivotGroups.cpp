...
int main(int argc, char* argv[]) {
	...
	getCommandOptions(argc, argv, inFileName, outFileName, K, groupSize);	
	...
	cout << "groupSize = " << groupSize << "\tK = " << K << endl;							
	printIdSet(seeds);		
	...
	for (it = seeds.begin(); it != seeds.end(); ++it) 
	{
		...
		do
		{
			...
			for (i = 0; i < numReads; ++i) 		
			{		
				dist = computeTotalKDist(readArray, K, tempSeeds, i);
...
void getCommandOptions(int argc, char* argv[], string &inFileName, string &outFileName, int &K, int &groupSize)
{
